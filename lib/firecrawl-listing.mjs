...
export async function fetchFirecrawlListing(sourceUrl, { apiKey = process.env.FIRECRAWL_API_KEY, fetchImpl = fetch } = {}) {
  ...
  try {
    ...
    // Temporary bounded source inspection while verifying the new provider.
    const $ = load(result.data.rawHtml || result.data.html || "");
    const heading = $("h1").first();
    const documentHtml = $.html();
    const headingHtml = $.html(heading);
    const sourceHeader = headingHtml ? documentHtml.slice(documentHtml.indexOf(headingHtml), documentHtml.indexOf(headingHtml) + 14000).split(/<h2\b/i)[0] : "";
    return { ok: true, property, source: "firecrawl", upstreamStatus: result.data?.metadata?.statusCode ?? null, sourceHeader };
